                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_START) {
        ap_started = 1;
        wifi_connected = 1;
        snprintf(local_ip, sizeof(local_ip), "%s", ap_ip);
        ESP_LOGI(TAG, "SoftAP started, IP: %s", local_ip);
        if (mdns_started && ap_netif != NULL)
        {
...
int wifi_init(const char *ssid, const char *password, const char *ap_ssid_in, const char *ap_password)
{
    if (ap_ssid_in == NULL || ap_password == NULL) {
        ESP_LOGE(TAG, "Invalid SoftAP credentials");
        return -1;
    }
    ...
    // Create default WiFi AP interface if it does not already exist.
    ...
    // Configure SoftAP only; this transmitter does not join a station network.
    ...
    ESP_LOGI(TAG, "SoftAP only WiFi initialization started");
...
int wifi_mdns_init(const char *hostname, uint16_t port)
{
    ...
    if (ap_netif != NULL && wifi_connected)
    {
        mdns_netif_action(ap_netif, MDNS_EVENT_ENABLE_IP4);
        mdns_netif_action(ap_netif, MDNS_EVENT_ANNOUNCE_IP4);
    }
